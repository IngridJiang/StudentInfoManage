 ...
 * @author gyt
 */
public class ShowcjtServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
       ...
       ResultSet rs1,rs2;
       String condition2="SELECT * from chengji where tID='"+gradeName+"'";
           rs2=sql.executeQuery(condition2);
       if(rs2.next()){ResultSetMetaData metaData = rs2.getMetaData();
          ...
          else {session.setAttribute("message", "试卷不存在");
                 response.sendRedirect("teacherchengji.jsp");
         }
        con.close();
